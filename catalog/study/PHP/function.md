...
| substr_replace | 替换字符串中的牟川为另一个字符串 | 将参数1从参数3开始(0起始)替换为参数2；参数4为替换的长度，如果为0可以当做插入字符串使用 |
...
| strpos | 寻找字符串中某字符穿最先出现的位置 | 搜索参数2在参数1最先出现的地方，【参数3可以指定起始搜索位置】 |
| stripos | 寻找字符串中某字符穿最先出现的位置，不区分大小写 | 该函数比起strpos其特点是不区分大小写的查找 |
